...
from .base_api import BaseTranslateAPI, TranslateError, DetectLanguage, Language, TranslateResult
...
class TranslateAPIProxyExecutor(BaseTranslateAPI):
    ...
    def _set_worker_info(self, proxy, proxy_cls, *proxy_cls_args,
                         **proxy_cls_kwargs):
        self.proxy = proxy
    ...
    def _init_worker(self):
        if self.proxy and self.proxy_cls:
            ...
            self._proxy_worker.set_proxy_info(self.proxy, self.proxy_cls,
                                              *self.proxy_cls_args,
                                              **self.proxy_cls_kwargs)
...
class ProxyAPIs(BaseTranslateAPI):
    ...
    def init(self, conf):
        ...
        for api_type, conf in self.conf.items():
            try:
                ...
                api = TranslateAPIProxyExecutor.create(
                    conf.get("proxy"), api_class,
                    conf) if "proxy" in conf else api_class(conf)
    ...
    async def async_translate_text(self, text, to_lang=None, **kwargs) -> TranslateResult:
        return await self.async_run(self.translate_text, text, to_lang,
                                    **kwargs)

    async def async_list_languages(self, display_language_code=None, **kwargs) -> list[Language]:
        return await self.async_run(self.list_languages, display_language_code,
                                    **kwargs)
        ...
        if self._default_api:
            try:
                ...
                raise TranslateError(
                    'Default API failed to detect language.') from exc
    ...
    def translate_text(self, text, to_lang=None, **kwargs):
        if not to_lang:
            ...
            if not from_lang:
                try:
                    from_lang = self.local_detect_language(
                        text, **kwargs).get('language_code')
                except Exception as e:
                    logging.warning(f'Failed to local detect language: {e}')
                    from_lang = self.detect_language(
                        text, **kwargs).get('language_code')
            to_lang = 'en' if from_lang.lower().startswith("zh") else 'zh'

        if self._default_api:
            try:
                return self._default_api.translate_text(
                    text, to_lang, **kwargs)
            except Exception as exc:
                raise TranslateError(
                    'Default API failed to translate text.') from exc
